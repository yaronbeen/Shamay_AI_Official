...
class BuildingExteriorAnalyzer {
    ...
    buildAnalysisPrompt(options = {}) {
        const hebrewPrompt = `נתח את תמונת החזית החיצונית של המבנה וספק ניתוח קצר בעברית:

**1. סוג המבנה**: סוג המבנה, מספר קומות, גיל משוער

**2. סגנון אדריכלי**: סגנון, חומרי בנייה, צבעים, אלמנטים מיוחדים

**3. מצב המבנה**: מצב כללי (מצוין/טוב/בינוני/דורש שיפוץ), חלונות, גג

**4. מרפסות**: סוג ומספר מרפסות, גודל, מצב

**5. חניה**: סוג פתרון חניה, נגישות, כניסה

**6. סביבה**: אופי השכונה, מבנים סמוכים, נוף

**7. אבטחה ותחזוקה**: מערכות אבטחה, איכות תחזוקה
