/**
 * Building Permits Module - Hebrew Building Permit Document Processing
 * 
 * Main module that provides PDF to markdown conversion and field extraction
 * for Hebrew building permit (היתרים מילוליים) documents using MarkItDown and custom parsers.
 */

const { convertPdfToMarkdown } = require('./pdf-converter.cjs');
const { FieldParser } = require('./field-parser.cjs');
const { AIFieldExtractor } = require('./ai-field-extractor.cjs');
...
async function processDocument(pdfPath, outputDir = 'output', options = {}) {
  try {
    ...
    const combinedResults = {
      ...
      fields: {
        permitNumber: extractionResults.permitNumber,
        permitDate: extractionResults.permitDate,
        permittedUsage: extractionResults.permittedUsage,
        permitIssueDate: extractionResults.permitIssueDate,
        localCommitteeName: extractionResults.localCommitteeName,
    ...
    console.log(`- Fields found: Permit=${extractionResults.permitNumber?.value}, Date=${extractionResults.permitDate?.value}, Committee=${extractionResults.localCommitteeName?.value}`);
