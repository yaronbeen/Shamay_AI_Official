    ...
    if (!shumaId) {
      ...
      await client.query(`
        UPDATE shuma SET
          ...
          rooms = CASE WHEN $6 IS NOT NULL AND $6::text != '' THEN $6::numeric ELSE rooms END,
          floor = COALESCE(NULLIF($7, ''), floor),
          air_directions = COALESCE(NULLIF($8, ''), air_directions),
          area = CASE WHEN $9 IS NOT NULL AND $9::text != '' AND $9::numeric != 0 THEN $9::numeric ELSE area END,
