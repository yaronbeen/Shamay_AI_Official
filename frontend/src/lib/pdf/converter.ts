...
export function convertValuationDataToReportData(
  ...
  const buildingPermit =
    extracted.building_permit || extracted.buildingPermit || {};
  const sharedBuildingOrder =
    extracted.shared_building_order || extracted.sharedBuildingOrder || {};
  ...
  const reportDate = formatDateHebrew(
    valuationData.valuationDate || new Date().toISOString(),
  );

  // Get reference number
  const referenceNumber =
    valuationData.referenceNumber ||
    `${valuationData.shamaySerialNumber || ""}_${fullAddress}`.replace(
      /[^a-zA-Z0-9_]/g,
      "_",
    );
  ...
  const permits = Array.isArray(buildingPermit)
    ? buildingPermit
  ...
  // Get comparable data
  const comparablesTable = (valuationData.comparableData || []).map(
    (comp: any) => ({
      ...
      pricePerSqmIls:
        comp.pricePerSqmIls ||
        comp.price_per_sqm ||
        (comp.priceIls || comp.price_ils || comp.price) /
          (comp.areaSqm || comp.area_sqm || comp.area || 1),
    }),
  );

  // Get valuation calculation
  const builtAreaSqm =
    valuationData.builtArea || extracted.built_area || valuationData.area || 0;
  const balconyAreaSqm =
    valuationData.balconyArea || extracted.balcony_area || 0;
  const equityAreaSqm =
    valuationData.registeredArea || extracted.registered_area || builtAreaSqm;
  const pricePerSqm =
    valuationData.pricePerSqm ||
    valuationData.finalValuation / equityAreaSqm ||
    0;
  const totalValue = valuationData.finalValuation || 0;

  // Get property photos
  const propertyPhotos = (valuationData.propertyImages || [])
  ...
  // Get air directions
  const airDirections =
    typeof valuationData.airDirections === "number"
  ...
  const easements =
    extracted.easements_list || landRegistry.easements_list || [];

  // Get building lines table
  const buildingLinesTable =
    extracted.building_lines_table ||
    rightsSummary.building_lines_table ||
    undefined;
  ...
  const buildingsInfo =
    extracted.buildings_info || sharedBuildingOrder.buildings_info || undefined;

  // Get appraiser license number
  const appraiserLicenseNumber =
    valuationData.appraiserLicenseNumber ||
    extracted.appraiser_license_number ||
    undefined;

  return {
    meta: {
      documentTitle:
        valuationData.valuationType || "אומדן שווי זכויות במקרקעין",
      reportDate,
      referenceNumber,
      clientName: valuationData.clientName || "",
      clientTitle: valuationData.clientTitle,
      inspectionDate: formatDate(
        valuationData.visitDate || valuationData.valuationDate,
      ),
      valuationDate: formatDate(
        valuationData.valuationDate || valuationData.visitDate,
      ),
    ...
    openingPage: {
      ...
      propertySummaryTable: {
        ...
        helka:
          ...
          "",
        area:
          valuationData.parcelArea ||
          extracted.parcel_area ||
          landRegistry.parcel_area,
      },
    },
    address: {
    ...
    cover: {
      ...
      companyServices:
        (companySettings as any)?.services?.join(" - ") ||
        "שמאות מקרקעין - התחדשות עירונית - דוחות אפס וליווי פיננסי - מיסוי מקרקעין",
      companyMembership:
        (companySettings as any)?.associationMembership ||
    ...
    section1: {
      ...
      environmentMap:
        valuationData.gisScreenshots?.wideArea ||
        valuationData.gisScreenshots?.cropMode0
          ? {
              src: (valuationData.gisScreenshots.wideArea ||
                valuationData.gisScreenshots.cropMode0) as string,
      ...
      parcel: {
        ...
        buildingYear: valuationData.buildingDescription
          ? parseInt(
              valuationData.buildingDescription.match(/\d{4}/)?.[0] || "",
            )
          : undefined,
        buildingFloors:
          valuationData.buildingFloors || extracted.building_floors,
        ...
        parcelSketch:
          valuationData.gisScreenshots?.zoomedWithTazea ||
          valuationData.gisScreenshots?.cropMode1
            ? {
                src: (valuationData.gisScreenshots.zoomedWithTazea ||
                  valuationData.gisScreenshots.cropMode1) as string,
        ...
        parcelSketchNoTazea: valuationData.gisScreenshots?.zoomedNoTazea
          ...
          : undefined,
        boundaries: Object.values(boundaries).some((v) => v)
          ? boundaries
          : undefined,
      },
      property: {
        subParcel:
          ...
          extracted.sub_parcel ||
          landRegistry.sub_parcel,
        rooms: valuationData.rooms || 0,
        floor: valuationData.floor || 0,
        airDirections,
        registeredAreaSqm:
          valuationData.registeredArea || extracted.registered_area,
        builtAreaSqm,
        balconyAreaSqm,
        attachmentsText,
    ...
    section2: {
      registryOfficeName:
        ...
        landRegistry.registry_office ||
        extracted.registry_office,
      tabuIssueDate: formatDate(
        valuationData.extractDate ||
          landRegistry.extract_date ||
          extracted.extract_date,
      ),
      easementsText:
        landRegistry.easements || extracted.easements || "אין זיקות",
      easements:
        easements.length > 0
          ? easements.map((easement: any, idx: number) => ({
              letter:
                easement.letter ||
                ["א", "ב", "ג", "ד", "ה", "ו", "ז"][idx] ||
      ...
      ownerships:
        ownerships.length > 0
          ...
          : undefined,
      mortgagesText:
        landRegistry.mortgages || extracted.mortgages || "אין משכנתאות רשומות",
      ...
      condoOrder: sharedBuildingOrder.order_date
        ? {
            ...
            buildingDescription:
              ...
              sharedBuildingOrder.building_description,
            buildingsInfo: buildingsInfo
              ? buildingsInfo.map((building: any) => ({
                  buildingNumber:
    ...
    section3: {
      plansTable: plansTable.map((plan: any) => ({
        ...
        publishDate: formatDate(
          plan.publication_date || plan.publishDate || plan.date,
        ),
        status: plan.status || "בתוקף",
      })),
      rightsSummary:
        Object.keys(rightsSummary).length > 0
          ? {
              designation: rightsSummary.designation || rightsSummary.usage,
              minLotSizeSqm:
                rightsSummary.minLotSizeSqm || rightsSummary.min_lot_size,
              buildPercentage:
                rightsSummary.buildPercentage || rightsSummary.build_percentage,
              maxFloors: rightsSummary.maxFloors || rightsSummary.max_floors,
              maxUnits: rightsSummary.maxUnits || rightsSummary.max_units,
              buildingLines:
                rightsSummary.buildingLines || rightsSummary.building_lines,
              buildingLinesTable: buildingLinesTable
                ? {
                    ...
                    side1:
                      buildingLinesTable.side1 ||
                      buildingLinesTable.צד1 ||
                      buildingLinesTable.צד_1,
                    side2:
                      buildingLinesTable.side2 ||
                      buildingLinesTable.צד2 ||
                      buildingLinesTable.צד_2,
                  }
                : undefined,
            }
          : undefined,
      auxiliaryAreas: auxiliaryAreas
        ? {
            parkingSpaces:
              auxiliaryAreas.parking_spaces ||
              auxiliaryAreas.parkingSpaces ||
              auxiliaryAreas.חניות,
            storageRooms:
              auxiliaryAreas.storage_rooms ||
              auxiliaryAreas.storageRooms ||
              auxiliaryAreas.מחסנים,
            otherAreas:
              auxiliaryAreas.other_areas || auxiliaryAreas.otherAreas
                ? (auxiliaryAreas.other_areas || auxiliaryAreas.otherAreas).map(
                    (area: any) => ({
      ...
      permits: permits.map((permit: any) => ({
        permitNumber:
          permit.permit_number || permit.permitNumber || permit.number || "",
        permitDate: formatDate(
          permit.permit_date || permit.permitDate || permit.date,
        ),
      ...
      apartmentPlan: valuationData.garmushkaMeasurements?.pngExport
        ...
        : undefined,
      environmentQualityText:
        extracted.land_contamination_note ||
    ...
    section4: {
      ...
      bullets: {
        ...
        rightsStatus: [
          ...
          valuationData.subParcel
            ? `הדירה זוהתה בהתאם לתשריט הבית המשותף כתת חלקה ${valuationData.subParcel} הנמצאת בקומה ${valuationData.floor || 0}.`
            : "",
        ].filter(Boolean),
        planningAndPermits: [
          'זכויות הבניה עפ"י תכניות בניין עיר בתוקף (כמפורט בפרק 3).',
          permits.length > 0
            ? `היתר בניה מס' ${permits[0].permitNumber} מיום ${permits[0].permitDate}.`
            : "",
    ...
    // Pass custom tables for DOCX export
    customTables: (valuationData as any).customTables || undefined,
  };
}
