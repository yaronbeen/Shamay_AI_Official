...
export const useShumaDB = () => {
  ...
  const saveShumaToDatabase = useCallback(async (sessionId: string, organizationId: string, userId: string, valuationData: ValuationData) => {
    setIsLoading(true)
    setError(null)
    try {
  ...
  const saveGarmushkaData = useCallback(async (sessionId: string, garmushkaData: any) => {
    setIsLoading(true)
    setError(null)
    try {
      const response = await fetch('/api/sessions', {
  ...
  const savePermitExtraction = useCallback(async (sessionId: string, permitData: any, documentFilename: string) => {
    setIsLoading(true)
    setError(null)
    try {
      const response = await fetch('/api/sessions', {
  ...
  const saveLandRegistryExtraction = useCallback(async (sessionId: string, landRegistryData: any, documentFilename: string) => {
    setIsLoading(true)
    setError(null)
    try {
      const response = await fetch('/api/sessions', {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Failed to save land registry extraction')
      }
  ...
  const saveSharedBuildingExtraction = useCallback(async (sessionId: string, sharedBuildingData: any, documentFilename: string) => {
    setIsLoading(true)
    setError(null)
    try {
      const response = await fetch('/api/sessions', {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Failed to save shared building extraction')
      }
  ...
  // Save final results
  const saveFinalResults = useCallback(async (sessionId: string, finalValuation: number, pricePerSqm: number, comparableData: any, propertyAnalysis: any) => {
    setIsLoading(true)
    setError(null)
    try {
      const response = await fetch('/api/sessions', {
  ...
  const getShumaById = useCallback(async (shumaId: string) => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/valuation-session?action=get_valuation&valuationId=${shumaId}`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' },
      })
  ...
  // Search shumas
  const searchShumas = useCallback(async (searchTerm?: string, status?: string) => {
    setIsLoading(true)
    setError(null)
    try {
      ...
      const response = await fetch(`/api/valuation-session?action=search_valuations&${params.toString()}`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' },
      })
