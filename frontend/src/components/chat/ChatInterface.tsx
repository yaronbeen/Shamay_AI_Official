...
export function ChatInterface({
  ...
  const containerClasses = {
    sidebar: 'h-full w-full bg-white flex flex-col',
    modal: 'fixed inset-4 z-50 rounded-lg shadow-2xl bg-white flex flex-col max-w-2xl mx-auto',
    embedded: 'h-full bg-white flex flex-col rounded-lg border border-gray-200 shadow-2xl',
  };
    ...
    <div className={`${containerClasses[mode]} ${className}`} dir="rtl">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gray-50">
        ...
        <div className="flex items-center gap-1">
          ...
          {/* Minimize button - hides the chat */}
          <button
            onClick={() => {
              setIsMinimized(true);
              onClose?.(); // Also call onClose to hide the chat window
            }}
            className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded transition-colors"
            title="מזער"
          >
