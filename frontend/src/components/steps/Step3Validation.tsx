...
export const filterPdfFiles = (
  files: Step3DocumentFile[],
): Step3DocumentFile[] => {
  return files.filter((f) => {
    ...
    const isPdf =
      ...
      name.endsWith(".pdf") ||
      url.endsWith(".pdf") ||
      ["tabu", "permit", "building_permit", "condo", "planning"].includes(
        rawType,
      );

    const isGarmushka =
      url.includes("garmushka") ||
      name.includes("garmushka") ||
      rawType.includes("garmushka");
    const isFromUploads =
...
export function Step3Validation({
  ...
  const [activeSection, setActiveSection] =
    useState<string>("fields_validation");
    ...
    const loadSessionData = async () => {
      ...
      try {
        ...
        if (
          ...
          const files = await getAllFilesFromSessionData(
            sessionData.data.uploads,
          );
          setAllFiles(files);
        }
            ...
            if (record.is_active && record.field_path) {
              if (!provenanceMap[record.field_path]) {
                ...
                provenanceMap[record.field_path] = {
                  ...
                  extractionMethod:
                    record.extraction_method === "manual" ? "manual" : "ai",
                  bbox: bbox,
                };
              }
            }
  ...
  const pdfFiles = useMemo(() => {
    ...
    console.log(
      "📁 [Step3] PDF files URLs:",
      filtered.map((f) => f.url),
    );
    return filtered;
  }, [allFiles]);
        ...
        <div className="bg-blue-50 border-b border-blue-200 p-4" dir="rtl">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">
            {aiExtractions.map((extraction, index) => (
              <div
                key={extraction.id}
                className="flex-shrink-0 bg-white rounded-lg p-3 border border-blue-200 min-w-[200px]"
              >
