...
export function Step4AIAnalysis({
  ...
  useEffect(() => {
    if (
      ...
      data.comparableDataAnalysis &&
      Object.keys(data.comparableDataAnalysis).length > 0
    ) {
  ...
  const handleMarketAnalysis = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/session/${sessionId}/comparable-data`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        },
      ...
      {sessionId ? (
        <ComparableDataViewer
          ...
          onAnalysisComplete={(analysis) => {
            console.log(
              "📊 Step4AIAnalysis: Comparable data analysis completed",
              analysis,
            );
            ...
            const avgPricePerSqm =
              typeof analysis.averagePricePerSqm === "string"
                ? parseFloat((analysis.averagePricePerSqm as string).trim())
                : typeof analysis.averagePricePerSqm === "number"
            ...
            const section52ValueNum =
              typeof section52Value === "string"
                ? parseFloat((section52Value as string).trim())
                : typeof section52Value === "number"
                  ? section52Value
                  : null;

            const estimatedValue =
              typeof analysis.estimatedValue === "string"
            ...
            updateData({
              ...
              marketAnalysis: {
                ...
                marketTrend:
                  (analysis as any).market_trends ||
                  data.marketAnalysis?.marketTrend ||
                  "",
            ...
            console.log(
              "✅ Step4AIAnalysis: Comparable data analysis updated in session and document data",
              {
                finalValuation,
      ...
      {sessionId ? (
        <GarmushkaMeasurementViewer
          ...
          onMeasurementComplete={(measurements) => {
            if (measurements) {
              console.log(
                "📐 Step4AIAnalysis: Garmushka measurements completed",
                measurements,
              );
              ...
              if (totalPolygonArea > 0) {
                updateObj.apartmentSqm =
                  Math.round(totalPolygonArea * 100) / 100;
                console.log(
                  `📐 Auto-updating apartmentSqm to ${updateObj.apartmentSqm} m²`,
                );
              }
              ...
              console.log(
                "✅ Step4AIAnalysis: Garmushka measurements updated in session",
              );
            } else {
              console.log(
                "📐 Step4AIAnalysis: Garmushka measurements returned null",
              );
            }
          }}
        />
  ...
  const renderAnalysisResults = () => (
    <div className="space-y-6">
      ...
      <p className="text-gray-600 text-sm mb-6">
        סקור וערוך את הניתוח שנוצר על ידי AI
      </p>
        ...
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div>
            ...
            <span className="font-semibold mr-2">
              ₪{analysisResults.marketAnalysis?.averagePricePerSqm || 0}
            </span>
          </div>
          <div>
            <span className="text-gray-500">מחיר חציוני למ"ר:</span>
            <span className="font-semibold mr-2">
              ₪{analysisResults.marketAnalysis?.medianPricePerSqm || 0}
            </span>
          </div>
          <div>
            <span className="text-gray-500">גורם התאמה:</span>
            <span className="font-semibold mr-2">
              {analysisResults.marketAnalysis?.adjustmentFactor || 1}
            </span>
          </div>
        </div>
      </div>
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-1 gap-4 sm:gap-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6 overflow-hidden relative">
          <h3 className="text-lg font-semibold text-gray-900 mb-4 text-right truncate">
            ניתוח AI
          </h3>
                ...
                >
                  <div className="flex flex-col items-center gap-2 sm:gap-3 min-w-0">
                    ...
                    <div className="text-center min-w-0 w-full">
                      <h4
                        className="font-semibold text-gray-900 truncate text-sm sm:text-base mb-1"
                        title={section.title}
                      >
                        {section.title}
                      </h4>
          ...
          <div className="border-t border-gray-200 pt-6 relative">
            ...
            {activeSection === "gis_mapping" && (
              <div
                role="tabpanel"
                id="panel-gis_mapping"
                aria-labelledby="tab-gis_mapping"
              >
            ...
            {activeSection === "garmushka_measurements" && (
              <div
                role="tabpanel"
                id="panel-garmushka_measurements"
                aria-labelledby="tab-garmushka_measurements"
              >
