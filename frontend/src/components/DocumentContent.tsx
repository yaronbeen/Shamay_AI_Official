...
export function DocumentContent({
  ...
  console.log(
    "- signaturePreview:",
    data.signaturePreview ? "Present" : "Missing",
  );
  console.log(
    "- selectedImagePreview:",
    data.selectedImagePreview ? "Present" : "Missing",
  );
  ...
  return (
    <div style={containerStyle} className="valuation-document">
      ...
      >
        ...
        <div style={{ fontSize: "10pt" }}>{getShamayLicense()}</div>
      </div>
      ...
      <div className="cover-page" style={{ marginBottom: "30px" }}>
        ...
        >
          <div>
            <strong>תאריך כתיבת השומה:</strong>{" "}
            {formatHebrewDate(data.valuationDate || new Date().toISOString())}
          </div>
          <div>
        ...
        <div style={{ marginBottom: "20px", fontSize: "11pt" }}>
          ...
          <div style={{ marginBottom: "10px" }}>
            <strong>מועד הביקור בנכס:</strong>{" "}
            {formatDate(
              data.valuationEffectiveDate || new Date().toISOString(),
            )}
          </div>
          <div>
            <strong>המועד הקובע לשומה:</strong>{" "}
            {formatDate(data.valuationDate || new Date().toISOString())}
          </div>
        </div>
        ...
        <div style={{ marginBottom: "20px", fontSize: "11pt" }}>
          <p>
            <strong>מטרת חוות הדעת:</strong> שומת מקרקעין בקריטריון של קונה
            מרצון למוכר מרצון (שווי שוק).
          </p>
          <p>
            <strong>הגבלת אחריות:</strong> אחריותו של החתום מטה מוגבלת למזמין
            השומה ולמטרת השומה בלבד. שימוש שלא בהתאם לאמור לעיל יעשה לאחר קבלת
            אישור מראש ובכתב מאת החתום מטה בלבד.
          </p>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="mb-4">
          ...
          <div className="bg-gray-50 p-3 rounded" style={{ fontSize: "11pt" }}>
            <p>
              הנכס ממוקם ב{getFullAddress()}, באזור מגורים מבוקש המאופיין באיכות
              חיים גבוהה ונגישות מעולה לתחבורה ציבורית. השכונה מציעה מגוון
              שירותים עירוניים, מוסדות חינוך איכותיים, מרכזי קניות ופנאי, וכן
              קרבה לאזורי תעסוקה מרכזיים. האזור נהנה מתשתיות מפותחות, כבישים
              רחבים ונגישות נוחה לכל חלקי העיר.
            </p>
        ...
        <div className="mb-4">
          ...
          <div className="bg-blue-50 p-3 rounded" style={{ fontSize: "11pt" }}>
            ...
            <p>
              על החלקה בניין מגורים{" "}
              {data.buildingDescription || "[תיאור/פירוט הבנייה]"}, אשר הוקם
              בהתאם ל{data.constructionSource || "[מקור הבניה, שנה]"}.
            </p>
            <p>
              הבניין בן {data.buildingFloors || "[מספר קומות]"} קומות
              {data.buildingDetails || "[ופירוט נוסף]"}, וכולל{" "}
              {data.buildingUnits || '[מספר יח"ד]'} יח"ד.
            </p>
        ...
        <div className="mb-4">
          ...
          <div className="bg-green-50 p-3 rounded" style={{ fontSize: "11pt" }}>
            <p>
              נשוא השומה הינה {data.subParcel || "[תיאור תת החלקה]"}, המהווה{" "}
              {data.propertyEssence || "[מהות הנכס]"}, הממוקמת בקומה{" "}
              {data.floor || "[קומה]"},
              {data.airDirections ? `פונה לכיוונים ${data.airDirections}` : ""}.
            </p>
            <p>
              הדירה בשטח רשום של {data.registeredArea || "[שטח רשום]"} מ"ר (נשלף
              אוטומטית מנסח הטאבו), ובשטח בנוי רישוי של כ-
              {data.builtArea || "[שטח בנוי]"} מ"ר (מוזן ידנית על ידי היוזר,
              עפ"י מדידה גרפית ע"ג תכנית היתר בניה מס'{" "}
              {data.buildingPermitNumber || "[מס' היתר]"} מיום{" "}
              {formatDate(data.buildingPermitDate || "")}).
      ...
      <div className="mb-6">
        ...
        <div className="mb-4">
          ...
          >
            <p>
              תמצית מידע מפנקס הזכויות המתנהל בלשכת רישום המקרקעין{" "}
              {data.registryOffice || "[שם הלשכה]"}
              (נשלף אוטומטית מהנסח), אשר הופק באמצעות אתר האינטרנט של רשם
              המקרקעין במשרד המשפטים, בתאריך{" "}
              {formatDate(data.extractDate || "")} (נשלף אוטומטית מהנסח).
            </p>
            <p>
              חלקה {data.parcel || "[מספר חלקה]"} בגוש{" "}
              {(data as any).block || data.gush || "[מספר גוש]"}, בשטח קרקע רשום
          ...
          !(data as any).riskAssessment && (
            <div className="space-y-2">
              ...
              <p className="text-xs" style={{ fontSize: "11pt" }}>
                הערכת השווי מבוססת על ניתוח השוואתי של נכסים דומים באזור, תוך
                התחשבות במאפייני הנכס, מיקומו ומצב השוק הנוכחי.
              </p>
            </div>
          )}
      </div>
        ...
        <div className="bg-gray-50 p-3 rounded" style={{ fontSize: "11pt" }}>
          ...
          <div className="space-y-3">
            <div>
              ...
              <p>
                • מיקום הנכס ב{getFullAddress()}, באזור מגורים מבוקש עם איכות
                חיים גבוהה
              </p>
            ...
            <div>
              ...
              <p>
                • הערכת שווי הנכס בוצעה בגישת ההשוואה, תוך ביצוע התאמות לשווי
                בהתחשב בפרמטרים ייחודיים לנכס
              </p>
      ...
      <div className="mb-6">
        ...
        <div className="mb-4">
          ...
          <div className="bg-blue-50 p-3 rounded" style={{ fontSize: "11pt" }}>
            <p className="mb-3">
              הובאו בחשבון נתוני עסקאות מכר של נכסים דומים רלוונטיים בסביבת נכס
              השומה, עפ״י דיווחים במערכת מידע-נדל״ן של רשות המיסים ומידע משלים
              מתוך היתרי הבניה.
            </p>
            ...
            (data as any).comparableData.length > 0 ? (
              ...
              <p className="text-gray-600">
                [נתוני השוואה יוצגו כאן לאחר העלאת קובץ CSV]
              </p>
            )}
          </div>
        </div>
      ...
      <div className="mb-6">
        ...
        >
          ...
          <p>
            הכול במצבו הנוכחי, כריק, פנוי וחופשי מכל מחזיק, חוב ושיעבוד, נכון
            לתאריך חוות-דעת זו.
          </p>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        >
          <p className="mb-4">
            הננו מצהירים, כי אין לנו כל עניין אישי בנכס נשוא השומה, בבעלי
            הזכויות בו במזמין השומה.
          </p>
          <p className="mb-4">
            הדו"ח הוכן על פי תקנות שמאי המקרקעין (אתיקה מקצועית), התשכ"ו – 1966
            ועל פי התקנים המקצועיים של הועדה לתקינה שמאית.
          </p>
          <p className="mb-4">ולראיה באנו על החתום,</p>

          {/* ✅ NEW: Signature Section */}
          <div className="mt-8 flex justify-between items-end">
            <div>
              <p className="mb-2">
                <strong>{getShamayName()}</strong>
              </p>
      ...
      >
        <p>
          דו"ח זה הוכן באמצעות מערכת SHAMAY.AI - פלטפורמה מקצועית להערכת שווי
          מקרקעין
        </p>
